...
import com.github.worldsender.mcanm.client.mcanmmodel.loader.VersionizedModelLoader;
 ...
 * useful if you later want to load the model with different data.
 *
 * @author WorldSEnder
 *
 */
public class MCMDModelLoader implements IResourceManagerReloadListener {
	/**
	 * This instance is registered in the {@link AdvancedModelLoader}-class provided by forge.
	 */
	...
	private IResourceManager getManager() {
		return manager == null ? Minecraft.getMinecraft().getResourceManager() : manager;
	}

	/**
	 * Used to load the models in an exception-free manner (for n0bs or just lazy guys who don't want to deal with
	 * exceptions). Static version of {@link #loadInstance(ResourceLocation)}
	 *
	 * @param resLoc
	...
	/**
	 * Actually loads the model. Note that this method will (normally) not throw. It catches {@link IOError}s as well as
	 * {@link ModelFormatException}, etc. This will always return a new Model but that may be empty (not render).<br>
	 * Convenience method, propagates to {@link #loadInstance(ResourceLocation, IResourceManager, boolean)} with the
	 * default {@link IResourceManager} and registers the model.
	 *
	 * @param resource
	...
	/**
	 * Actually loads the model. Note that this method will (normally) not throw. It catches {@link IOError}s as well as
	 * {@link ModelFormatException}, etc. This will always return a new Model but that may be empty (not render).
	 *
	 * @param resource
	 *            the resource to load this model from
	 * @param resManager
	 ...
	 * @param register
	 *            if true the loaded model will be registered to reload on switching resourcepack
	 * @return the newly loaded {@link ModelMCMD}, never <code>null</code>
	 */
	...
	/**
	 * ADVANCED USE<br>
	 * Use this to load a Model and receive any occuring Exceptions. The <<<<<<< HEAD received model can later be
	 * registered in the {@link ModelLoader} to ======= received model can later be registered in the
	 * {@link ModelRegistry} to >>>>>>> branch '1.8' of https://github.com/WorldSEnder/MCAnm cache it.<br>
	 * This does also NOT bake the model, you have to do that later on
	 *
	 * @param resLocation
	 *            the {@link ResourceLocation} to load from
	 * @param resManager
	 *            the {@link IResourceManager} to use during loading. You may use
	 *            <code>Minecraft.getMinecraft().getResourceManager()</code> .
	...
	/**
	 * ADVANCED USE<br>
	 * Loads the model from an already open {@link DataInputStream}. This method will - unlike
	 * {@link #loadInstance(ResourceLocation)} - throw when a {@link ModelFormatException} is encountered because it is
	 * for advanced users.<br>
	 * The model is not scheduled for reloading when the user switches his/her resourcepack.
	 *
	 * @param dis
