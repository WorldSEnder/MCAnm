...
/**
 * A central place to load animations from. Not only does this class provide the utility of guaranteed no-throw method
 * but also reuses already loaded animations if requested twice.
 *
 * @author WorldSEnder
 *
 */
...
public class AnimationLoader {
	 ...
	 * If an animation for the requested {@link ResourceLocation} has already been registered the registered one is
	 * returned instead.<br>
	 * This method catches {@link ModelFormatException}s and any other occurring Exceptions and logs them with
	 * {@link MCAnm#logger}. This method will then return <code>null</code> and the animation will not be registered.
	 * <br>
	 *
	 * @param resLoc
	 *            the requested {@link ResourceLocation}
	 * @return an animation that has been loaded from that {@link ResourceLocation}
	 */
	public static StoredAnimation loadAnimation(ResourceLocation resLoc) {
		return instance.loadAnimation(resLoc, Minecraft.getMinecraft().getResourceManager());
	}
	...
	/**
	 * Same as {@link #loadAnimation(ResourceLocation)} but if no animation has been registered for the given
	 * {@link ResourceLocation} it is loaded using the {@link IResourceManager} given.
	 *
	 * @param resLoc
	 *            the requested {@link ResourceLocation}
	 * @param resManager
	...
	public StoredAnimation loadAnimation(ResourceLocation resLoc, IResourceManager resManager) {
		try {
			...
			MCAnm.instance.getLogger().error(String.format("Error loading animation %s", resLoc), mfe);
		}
		return null;
	}

	/**
	 * Registers the given animation in the registry. The animation has to be constructed with either the
	 * {@link #StoredAnimation(ResourceLocation)} or {@link #StoredAnimation(ResourceLocation, IResourceManager)}, else
	 * an invalid argument exception is being thrown. The animation shall not be <code>null</code>.<br>
	 * This method does nothing if an animation with the same origin has been registered before.
	 *
	 * @param animation
	 *            the animation to register
	 * @return true if no animation for the {@link ResourceLocation} from which <code>animation</code> has been loaded
	 *         from has been registered before
	 */
