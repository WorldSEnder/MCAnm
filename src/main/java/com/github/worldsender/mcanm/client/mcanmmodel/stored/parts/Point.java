...
import com.github.worldsender.mcanm.client.mcanmmodel.stored.RawDataV1.TesselationPoint;
...
public class Point {
		...
		public BoundPoint(
			...
			for (RawDataV1.BoneBinding bind : readBinds) {
				if (bind.bindingValue <= 0.0f)
					continue;
				this.binds.add(new Binding(skelet.getBoneByIndex(bind.boneIndex & 0xFF), bind.bindingValue));
	 ...
	 */
	public static Point from(RawDataV1.TesselationPoint data, ISkeleton skelet) {
